...
@Component
public class AddressTypeProcessor extends BaseEntityProcessor {

    public AddressTypeProcessor(
    ...
    public void process(DBFRow row) {
        MigrationConfig.EntityConfig config = migrationConfig.getEntities().get("addressType");

        Map<String, Object> values = process(row, config);

        cashes.get("addressTypeCache").add(values.get("caption").toString(), UUID.fromString(values.get("id").toString()));
