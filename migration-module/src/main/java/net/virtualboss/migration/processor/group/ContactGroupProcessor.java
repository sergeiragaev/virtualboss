...
@Component
public class ContactGroupProcessor extends BaseEntityProcessor {

    public ContactGroupProcessor(
    ...
    public void process(DBFRow row) {
        MigrationConfig.EntityConfig config = migrationConfig.getEntities().get("contactGroup");

        Map<String, Object> values = process(row, config);

        cashes.get("contactGroupCache").add(values.get("name"), UUID.fromString(values.get("id").toString()));
