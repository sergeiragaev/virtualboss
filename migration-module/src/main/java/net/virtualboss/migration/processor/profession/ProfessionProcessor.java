...
@Component
public class ProfessionProcessor extends BaseEntityProcessor {

    public ProfessionProcessor(
    ...
    public void process(DBFRow row) {
        MigrationConfig.EntityConfig config = migrationConfig.getEntities().get("profession");

        Map<String, Object> values = process(row, config);

        cashes.get("professionCache").add(values.get("name").toString(), UUID.fromString(values.get("id").toString()));
